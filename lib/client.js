...
Client.prototype.onError = function onError(error)
{
    ...
    this.rejectAllQueues(error);
    this.isConnected = false;
    this.isReady = false;

    var socketError = new Error('Unexpected socket error.');
    _.assign(socketError, { 'error': error });
...
Client.prototype.authenticateWith = function authenticateWith(password)
{
    ...
    }, function(error)
    {
        ...
        if (!/No password is set/i.test(error))
            throw error;
        return 'OK';
    });
};
...
Client.prototype.send = function send(name, parameters)
{
    ...
    if (!length)
    {
        var argumentError = new Error('Missing command name.');
    ...
    else
    {
        ...
        if (length > 2)
            parameters.push.apply(parameters, _.rest(arguments, 2));

        var lastParameter = parameters.pop(),
            isHashSet = /^hmset$/i.test(name);

        if (Array.isArray(lastParameter) && (isHashSet || /^s(?:add|rem)$/i.test(name)))
            parameters.push.apply(parameters, lastParameter);
        else if (isHashSet && _.isObject(lastParameter))
        ...
        else
        {
            ...
            {
                var argumentError = new Error('`set` and `setex` require additional arguments.');
...
Client.prototype.connect = function connect()
{
    ...
    var isErrorHandlerAttached = true,
        errorHandler = _.bindKey(self, 'onError');
...
Client.prototype.transact = function transact()
{
    var self = this,
    ...
    [
        ...
        {
            return Array.isArray(command) ? self.send.apply(self, command) : self.send(command);
        })),
        this.send('exec')
...
{
    var self = this,
        parameters = ['evalsha', crypto.createHash('sha1').update(source).digest('hex')];

    parameters.push.apply(parameters, _.rest(arguments));

    return this.send.apply(this, parameters).fail(function(error)
    {
        if (/noscript/i.test(error))
        {
            parameters.splice(0, 2, 'eval', source);
