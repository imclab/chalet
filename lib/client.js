...
Client.prototype.onReady = function onReady()
{
    ...
    var promise = this.isMonitoring === true ? this.send('monitor') : this.sendOfflineQueue();
...
Client.prototype.sendQueue = function sendQueue(offlineQueue)
{
    ...
    while (offlineQueue.length)
    {
        var command = offlineQueue.shift(),
            writePromise = this.writeCommand(command);

        writePromises.push(writePromise);
        promises.push(command.promise);
    }

    return P.all(writePromises).then(function()
    {
        return promises;
    }, function(error)
    {
...
Client.prototype.sendAuthQueue = function sendAuthQueue()
{
    ...
    if (authQueue.length)
    {
        return this.sendQueue(authQueue).then(function(promises)
        {
            return P.all(promises);
        }).then(_.bindKey(this, 'handleAuthenticate'));
    }

    return this.authenticateWith(this.password).then(_.bindKey(this, 'handleAuthenticate'));
};
...
Client.prototype.send = function send(command)
{
    ...
    if (parameters.length > 1)
    {
        ...
        if (/^s(?:add|rem)$/i.test(command) && Array.isArray(lastParameter))
            parameters.push.apply(parameters, lastParameter);
        else
        {
            parameters.push(lastParameter);
            if (/^set(ex)?$/i.test(command) && lastParameter == null)
            {
                var argumentError = new Error('This command requires an argument value.');
                _.assign(argumentError, { 'name': command, 'parameters': parameters });
    ...
    if (/^hmset$/i.test(command) && parameters.length == 2)
    {
        var lastParameter = parameters.pop();
    ...
    var commandObject = new Command(command, parameters),
        writePromise = this.writeCommand(commandObject);

    return writePromise.then(function()
    {
        return commandObject.promise;
    });
};

Client.prototype.writeCommand = function writeCommand(commandObject)
{
    var self = this,
    ...
    if (isOffline && !this.enableOffline)
    {
        var connectionError = new Error('Offline queueing has been disabled for this connection.'),
            connectionDeferred = P.defer();

        _.assign(connectionError, { 'command': commandObject, 'isReady': isReady, 'isOffline': isOffline });
        connectionDeferred.reject(connectionError);
        return connectionDeferred.promise;
    }

    if (isAuthentication && !commandObject.isAuthentication && !isWritable)
    {
        this.authQueue.push(commandObject);
        commandObject.isAuthentication = true;
        return commandObject.promise;
    }

    if (isOffline)
    {
        this.offlineQueue.push(commandObject);
        return P(false);
    }

    if (/^p?(un)?subscribe$/i.test(name))
        this.updateSubscribers(commandObject);
    ...
    else if (this.isSubscriber)
    {
        var modeError = new Error('A client in subscriber mode cannot issue commands.'),
            modeDeferred = P.defer();

        _.assign(modeError, { 'command': commandObject });
        modeDeferred.reject(modeError);
        return modeDeferred.promise;
    }

    var name = commandObject.name;
    if (/^exec$/i.test(name))
    {
        this.isQueueing = false;
        commandObject.endsQueue = true;
    }
    else if (/^discard$/i.test(name))
    {
        this.isQueueing = false;
        commandObject.discardsQueue = true;
    }
    else if (this.isQueueing)
    {
        this.transactionQueue.push(commandObject);
        commandObject.isQueued = true;
    }
    else if (/^multi$/i.test(name))
    {
        this.isQueueing = true;
        commandObject.beginsQueue = true;
    }

    return P(commandObject).then(function(commandObject)
    {
        self.commandQueue.push(commandObject);
        self.commandsSent++;
        return commandObject.writeTo(self.socket);
    });
};
...
Client.prototype.connect = function connect()
{
    ...
    socket.on('connect', _.bindKey(this, 'onConnect')).once('connect', deferred.resolve);
    socket.on('error', _.bindKey(this, 'onError')).once('error', deferred.reject);

    socket.on('data', _.bindKey(this, 'onData'));
    socket.on('drain', _.bindKey(this, 'onDrain'));
    socket.on('close', _.bindKey(this, 'onClose', 'close')).on('end', _.bindKey(this, 'onClose', 'end'));
...
Client.prototype.transact = function transact(commands)
{
    ...
    [
        ...
        P.allSettled(_.map(commands, function(command)
        {
            ...
            return self.send.apply(self, [name].concat(parameters));
        })),
        this.send('exec')
