...
Client.prototype.sendAuthQueue = function sendAuthQueue()
{
    var authQueue = this.authQueue,
        promises = authQueue.length ? this.sendQueue(authQueue) : [this.authenticateWith(this.password)].concat(this.sendQueue(authQueue));

    return P.all(promises).then(_.bindKey(this, 'handleAuthenticate'));
};
...
Client.prototype.send = function send(command)
{
    ...
    var commandObject = new Command(command, parameters),
        bufferedWrite = this.writeCommand(commandObject);
...
Client.prototype.writeCommand = function writeCommand(commandObject)
{
    ...
    var bufferedWrites = commandObject.writeTo(this.socket);
