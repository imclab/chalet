...
{
    var self = this,
        parameters = [this.name].concat(this.parameters);

    var headerDeferred = P.defer();
    socket.write('*' + parameters.length + '\r\n', headerDeferred.resolve);

    return headerDeferred.promise.then(function()
    {
        var hasBuffers = self.hasBuffers = _.some(self.parameters, Buffer.isBuffer);

        if (!hasBuffers)
        {
            // Fast path: no buffers; serialize the arguments and make one write
            // to the socket.
            var bodyDeferred = P.defer();

            var commandBody = _.map(parameters, function(parameter)
            {
                var argument = String(parameter);
                return '$' + Buffer.byteLength(argument) + '\r\n' + argument;
            }).join('\r\n');

            socket.write(commandBody + '\r\n', bodyDeferred.resolve);
        ...
        return P.all(_.map(parameters, function(parameter)
        {
            var bodyDeferred = P.defer(),
                isBuffer = Buffer.isBuffer(parameter),
            ...
            if (!isBuffer || !argument.length)
            {
                ...
                socket.write(bodyString, bodyDeferred.resolve);
                return bodyDeferred.promise;
            }

            var beginPaddingDeferred = P.defer(),
                endPaddingDeferred = P.defer();

            socket.write('$' + argument.length + '\r\n', beginPaddingDeferred.resolve);
            socket.write(argument, bodyDeferred.resolve);
            socket.write('\r\n', endPaddingDeferred.resolve);

            return P.all([beginPaddingDeferred.promise, bodyDeferred.promise, endPaddingDeferred.promise]);
        }));
    });
};
